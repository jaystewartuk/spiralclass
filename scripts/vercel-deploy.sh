...
#   3. `vercel deploy --prebuilt --skip-domain --no-wait` — a production
#      deployment that does NOT take the domain — and scripts/vercel-await.mjs
#      to wait for it, with a deadline and Vercel's reason on any failure
...
#
# --no-wait, and scripts/vercel-await.mjs does the waiting. The CLI's own wait
# has no deadline and leaves only on READY, ERROR or CANCELED — so when Vercel
# BLOCKED the deployment (its commit-author check, 2026-09-18), the CLI
# printed "Building…" until the job's timeout killed it, and the log never said
# what Vercel had decided. The await fails on every state it does not know to
# be in progress, within a bounded time, with Vercel's reason.
#
# The CLI's exit status is kept rather than tripping `set -e`: when it fails
# after creating the deployment, the URL is already on stdout, and the await is
# what can say why. A non-zero status still fails the deploy below.
echo "› Deploying the prebuilt output (--skip-domain: spiralclass.com stays on Fly)…"
DEPLOY_STATUS=0
DEPLOYMENT_URL="$("${VERCEL[@]}" deploy --prebuilt --prod --skip-domain --no-wait --local-config "$CONFIG")" ||
  DEPLOY_STATUS=$?
if [ -z "$DEPLOYMENT_URL" ]; then
  echo "vercel-deploy: the CLI created no deployment (exit ${DEPLOY_STATUS}) — its error is above." >&2
  exit 1
fi

echo "› Created ${DEPLOYMENT_URL} — waiting for Vercel to finish it…"
node scripts/vercel-await.mjs "$DEPLOYMENT_URL"
if [ "$DEPLOY_STATUS" -ne 0 ]; then
  echo "vercel-deploy: the CLI exited ${DEPLOY_STATUS} although the deployment finished — refusing to call it deployed." >&2
  exit "$DEPLOY_STATUS"
fi
