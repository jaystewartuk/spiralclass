...
#
# A heredoc body is DATA the command writes, not command the shell runs, and
# the rules below could not tell the two apart. The first casualty was a commit
# message: `git commit -F - <<'MSG'` whose message explained a change to
# push-cloudrun-env.sh was blocked as a deploy of it. A guard that blocks
# writing ABOUT the deploy path is not stricter, it is wrong — and it teaches
# the reflex of working around the guard, which is the one habit this file
# exists to prevent.
#
# Stripping the body rather than loosening the match is the narrower fix: every
# rule below keeps matching its name ANYWHERE in the real command, so nothing
# that was blocked before stops being blocked. Only the prose goes.
#
# The line that OPENS the heredoc is kept — `git commit -F -` is a command, and
# a redirect on that line still deserves to be read.
strip_heredocs() {
  local line tag="" out=""
  while IFS= read -r line; do
    if [[ -n "$tag" ]]; then
      # A `<<-` heredoc allows leading tabs before the terminator.
      [[ "${line//[$'\t ']/}" == "$tag" ]] && tag=""
      continue
    fi
...
command_only="$(strip_heredocs "$command_line")"
...
# gcloud (describe, list, logging read) is deliberately NOT matched: measuring
# the target is how the decision behind it was taken.
if [[ "$command_only" == *cloudrun-deploy.sh* ]] ||
  [[ "$command_only" == *push-cloudrun-env.sh* ]] ||
  [[ "$command_only" == *setup-deploy-identity.sh* ]] ||
  [[ "$command_line" =~ (^|[[:space:]&|;])gcloud[[:space:]]+(run[[:space:]]+(deploy|services[[:space:]]+(replace|update|delete)|domain-mappings)|secrets[[:space:]]+(create|versions|delete)|iam[[:space:]]+service-accounts[[:space:]]+keys) ]]; then
  block "This deploys to, writes a secret for, or mints a key for the live Cloud Run target." \
    "It is the third production target ([D-184]) and the one built to take the domain.
Production only ever ships through 'pnpm promote', run by the operator, and the
infra/gcp/ scripts are the operator's own."
fi

# The database half of every deploy, on its own since the production targets
# were split ([D-177]'s addendum). Matched on the name anywhere in the COMMAND,
# like the Fly script: `bash -c` or a path prefix is a likelier way a session
# would reach it than a bare invocation.
#
# ⚠️ The original reason given for matching anywhere was that "nothing ordinary
# spells it". That turned out to be false — a commit message explaining a change
# to one of these scripts spells it, and was blocked as a deploy. Hence
# `$command_only`: the name still matches anywhere a command could run, and no
# longer matches inside a heredoc body. See strip_heredocs above.
if [[ "$command_only" == *database-deploy.sh* ]]; then
