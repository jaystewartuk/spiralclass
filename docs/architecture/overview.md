...
constraint — a UK platform cannot `Transfer` to a connected account outside its payout circle,
and that is where the first teachers were. The fix was to stop needing a payout
...
locale was shipped and was wrong: the platform's first teacher reads Spanish
...
- **Platform entity**: consolidated in the UK
  ([D-58](../decisions/D-58.md)), and platform billing moved to GBP
