...
// ICU than Node, and the runtime therefore never calls Intl; it reads a table.
//
...
// Spanish and essentially none of which anyone teaches. A language outside
// 639-1 that a teacher does teach would need a hand-curated row, because CLDR
// has no name for most of them (probe `nah`, `yua`, `mix` — all undefined).
...
const es = new Intl.DisplayNames(["es"], { type: "language", fallback: "none" });
...
const body = rows
  .map(
    (r) =>
      `  { code: ${JSON.stringify(r.code)}, label: { en: ${JSON.stringify(r.en)}, "es": ${JSON.stringify(r.es)} }, asr: ${r.asr}, gen: ${r.gen} },`,
  )
  .join("\n");
