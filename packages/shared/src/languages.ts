...
export type Language = {
  ...
  gen: boolean;
  // True for a macrolanguage: one code standing for variants that are often
  // NOT mutually intelligible. Picking the macro is valid — it means "the
  // language, variant unspecified". No generated row sets it today.
  macro?: boolean;
  // The macrolanguage this row is a variant of. Set on variants only; would
  // drive a two-level picker. No row sets it today; the structure exists so
  // adding variants is data, not a schema change.
  parent?: string;
};

export const LANGUAGES: readonly Language[] = WORLD_LANGUAGES;
...
/**
 * Localized display name, e.g. languageLabel("pt", "es") → "Portugués".
