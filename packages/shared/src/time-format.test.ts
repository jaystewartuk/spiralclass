...
// 17:00 in San José on a winter Tuesday, and 09:00 the same morning.
...
describe("hourStyleFor", () => {
  ...
  it("drops it on a 12-hour locale, where the meridiem has already spent the width", () => {
    // BOTH of the registry's other locales, es included. That locale writes
    // "5:00 p.m." — CLDR puts es on h12, not h23. That is the fact an
    ...
    expect(hourStyleFor("es")).toBe("numeric");
  });
...
describe("formatTimeInZone", () => {
  it("writes five in the afternoon the way an English reader writes it", () => {
    ...
    expect(formatTimeInZone(AFTERNOON, CR, "en-US")).toBe("5:00 PM");
    expect(formatTimeInZone(MORNING, CR, "en-US")).toBe("9:00 AM");
  });

  it("writes it the way an es reader writes it", () => {
    // The change the platform's one Spanish-reading teacher actually sees:
    // "05:00 p.m." → "5:00 p.m.".
    expect(formatTimeInZone(AFTERNOON, CR, "es")).toBe("5:00 p.m.");
    expect(formatTimeInZone(MORNING, CR, "es")).toBe("9:00 a.m.");
  });

  it("leaves a 24-hour locale exactly as it was", () => {
    expect(formatTimeInZone(AFTERNOON, CR, "fr")).toBe("17:00");
    expect(formatTimeInZone(MORNING, CR, "fr")).toBe("09:00");
  });
...
describe("timeOptionsFor", () => {
  it("spreads into a larger options bag without carrying a date with it", () => {
    ...
    expect(timeOptionsFor("es")).toEqual({ hour: "numeric", minute: "2-digit" });
  });
