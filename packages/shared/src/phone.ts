...
//      landline really is "+39 06…" — so it is excluded. +52's legacy
//      "044"/"045"/"01" mobile prefixes are their own case, and are stripped
//      ONLY for +52: the same pattern applied to a French "01 42 86 80 00" ate
...
// historical +52-first heuristic is kept as the last resort (a bare
// 10-digit number defaults to +52) purely so pre-existing +52 rows and call
 ...
 * (not every country's national number is 10 digits, unlike +52's).
 * Omit it (or pass an unmapped country) to keep the historical +52-default
 * 10-digit heuristic.
 */
export function normalizeE164(input: string, countryHint?: string): string {
  ...
  if (country === "MX") {
    // Legacy +52 trunk / mobile prefixes dialed before the 10-digit national
    // number ("044 55…", "045 55…", "01 55…"). Scoped to +52 deliberately: the
    ...
    // country with no trunk prefix (the NANP, +52, ES, PT…) never presents a
  ...
  // digits) — most common for the +52 fallback; other countries' national
  // numbers vary in length, so this check is a shortcut, not a requirement.
  if (s.startsWith(cc) && s.length === cc.length + 10) return `+${s}`;
  if (hinted) {
    // An explicit, user-declared country: trust it over the +52-specific
    // 10-digit heuristic below.
    return `+${cc}${s}`;
  }
  // No usable hint — fall back to the historical +52 heuristic: a plain
