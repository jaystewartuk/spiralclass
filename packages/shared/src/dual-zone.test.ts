...
describe("getDualZoneTime", () => {
  ...
  it("UK <-> UTC-6: viewer primary is UTC-6, other secondary is UK", () => {
    // 2026-07-15 16:00 UTC -> San José (UTC-6) 10:00 AM, London (BST, UTC+1) 5:00 PM
    const d = new Date("2026-07-15T16:00:00Z");
    const dz = getDualZoneTime(d, student("America/Costa_Rica"), teacher("Europe/London"), "en", d);
  ...
  it("DST transition: same IANA zone, one instance before and one after UK clocks change", () => {
    ...
    // local, but different UTC instants and a shifted UTC-6 equivalent.
    ...
    // San José has no DST — the equivalent local time shifts by an hour
  ...
  it("midnight-boundary: calendar date differs between viewer and other", () => {
    // 2026-07-15 23:30 UTC -> San José (UTC-6) still 2026-07-15 17:30,
    // Tokyo (UTC+9) already 2026-07-16 08:30.
    const d = new Date("2026-07-15T23:30:00Z");
    const dz = getDualZoneTime(d, student("America/Costa_Rica"), teacher("Asia/Tokyo"), "en", d);
    expect(dz.sameCalendarDate).toBe(false);
  });

  it("uses the viewer's own 'today' reference, not the other party's", () => {
    const now = new Date("2026-07-15T12:00:00Z");
    // A class at 23:30 UTC on 2026-07-15 is still "today" for the UTC-6
  ...
  it("names today and tomorrow in EVERY registered locale, not just two", () => {
    ...
    const parties = [student("America/Costa_Rica"), teacher("America/Costa_Rica")] as const;
  ...
  it("falls back to full IANA id when ICU has no real abbreviation (ambiguous offset)", () => {
    const d = new Date("2026-07-15T16:00:00Z");
    const dz = getDualZoneTime(d, student("America/Costa_Rica"), teacher("Europe/London"), "en", d);
    // A UTC-6 zone commonly resolves to "GMT-6" under small-icu builds — must not surface that as-is.
    expect(
      dz.viewer.tzDisplay === "America/Costa_Rica" || /^[A-Z]{2,5}$/.test(dz.viewer.tzDisplay),
    ).toBe(true);
  });
});
