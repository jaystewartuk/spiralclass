...
//                            longer binds. The launch market, Brazil, Japan, Singapore,
...
export function countryLabel(code: string, locale: LocaleCode = "en"): string {
  ...
  try {
    ...
    const dn = new Intl.DisplayNames([intlLocale(locale)], { type: "region", fallback: "none" });
    return dn.of(c) ?? c;
  } catch {
    return c;
  }
 ...
 * language (an en-US browser in Brazil still reports America/Sao_Paulo), and
 ...
 * returns null so the caller can fall back to the locale region or leave the
 * field for the teacher. Not exhaustive by design — a suggestion, always user-correctable.
 */
...
//   node --input-type=module -e 'const en=new Intl.DisplayNames(["en"],{type:"region",fallback:"none"});const es=new Intl.DisplayNames(["es"],{type:"region",fallback:"none"});for (const c of CODES) console.log(c, en.of(c), es.of(c))'
