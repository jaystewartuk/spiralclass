...
export const timezoneSchema = (locale: AppLocale = "en") => {
  ...
  return z.object({
    ...
    // the schema layer so a pre-existing client that predates this field
    // keeps working — it omits it and the teacher stays on the column
    // default. Locked after onboarding — see D-64.
    pricingCurrency: z
      .string()
      .trim()
      .toUpperCase()
...
export const leadCaptureSchema = (locale: AppLocale = "en") => {
  ...
  return z.object({
    ...
    // (older client) falls back to normalizeE164's +52 default.
    phoneCountry: z
      .string()
      .trim()
      .optional()
