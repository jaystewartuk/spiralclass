...
describe("isConnectCountrySupported", () => {
  ...
  it("accepts the launch market, BR, JP and the rest of the non-circle markets (D-143)", () => {
    ...
    for (const code of ["BR", "JP", "SG", "TH", "AU", "NZ", "HK", "MY", "AE", "MX"]) {
      expect(isConnectCountrySupported(code)).toBe(true);
    }
  });
...
describe("country catalog", () => {
  ...
  it("includes the markets the platform sells into", () => {
    for (const code of ["US", "GB", "BR", "ES", "JP", "CO"]) {
...
describe("countryLabel / countryOptions", () => {
  ...
  it("never returns a bare 2-letter code for a real country", () => {
    ...
    for (const code of ["AD", "AE", "AF", "AR", "BR", "JP", "DE"]) {
      ...
      expect(countryLabel(code, "es")).not.toBe(code);
    }
  });
...
describe("filterCountryOptions", () => {
  ...
  it("matches on the ISO code too", () => {
    expect(filterCountryOptions(opts, "de").map((o) => o.code)).toEqual(["DE"]);
  });
...
describe("countryFromTimezone", () => {
  ...
  it("only ever maps to real ISO codes we actually offer", () => {
    ...
    for (const tz of ["America/Costa_Rica", "Europe/Madrid", "Asia/Kolkata", "Africa/Lagos"]) {
