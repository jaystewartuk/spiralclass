...
describe("normalizeE164", () => {
  ...
  it("keeps a plus-prefixed number's own country code (does not force +52)", () => {
    // A US number the sender explicitly qualified with +1.
    expect(normalizeE164("+1 555 123 4567")).toBe("+15551234567");
  });

  it("prepends + to a number that already leads with the +52 country code", () => {
    expect(normalizeE164("525512345678")).toBe("+525512345678");
  });

  // The core regression: a bare, country-code-less 10-digit number must land on +52,
  // not be mis-stamped +55 (Brazil) by a blind "+" prepend.
  it("defaults a bare 10-digit national number to +52", () => {
    expect(normalizeE164("55 1234 5678")).toBe("+525512345678");
    expect(normalizeE164("(55) 1234-5678")).toBe("+525512345678");
    expect(normalizeE164("5512345678")).toBe("+525512345678");
  });

  it("strips the legacy +52 trunk/mobile prefixes (044/045/01)", () => {
  ...
  describe("countryHint", () => {
    ...
    it("still defaults to +52 when no hint is given (unchanged behavior)", () => {
      expect(normalizeE164("5512345678")).toBe("+525512345678");
    });

    it("falls back to the +52 default for an unmapped/unknown country hint", () => {
    ...
    it("still strips the legacy +52 trunk prefix when the matching hint is given", () => {
    ...
    // The bug the single-country scoping fixes: `^0(44|45|1)` applied to every
    // country ate the "01" area code off a French landline, producing a
    // working number belonging to a stranger.
    it("does not apply the legacy +52 trunk strip to another country", () => {
...
describe("splitE164", () => {
  ...
  it("strips the +52 calling code, matching the default hint", () => {
  ...
  it("falls back to matching by calling code when the hint doesn't match the number", () => {
    // A GB student's number saved while the teacher's own default hint is +52.
