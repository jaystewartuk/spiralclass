...
describe("locale registry", () => {
  ...
  it("keeps every locale key-complete against the source locale", () => {
    ...
    const sourceKeys = Object.keys(strings["es"]).sort();
    for (const locale of LOCALES) {
  ...
  it("matchAcceptLanguage picks the locale for a header prefix", () => {
    expect(matchAcceptLanguage("es,es;q=0.9")).toBe("es");
  ...
  it("bridges locales to the languageCode used by outbound copy", () => {
    expect(localeToLanguageCode("es")).toBe("es");
  ...
  it("languageCodeToLocale bridges the languageCode back to a tag", () => {
    expect(languageCodeToLocale("es")).toBe("es");
...
describe("language preference (System Default)", () => {
  ...
  it("resolveLocale follows the device for 'system', pins an explicit locale", () => {
    ...
    expect(resolveLocale("es", "fr")).toBe("es");
  });
...
describe("createT / interpolate", () => {
  it("resolves a key in the requested locale", () => {
    expect(createT("es")("common.save")).toBe("Guardar");
