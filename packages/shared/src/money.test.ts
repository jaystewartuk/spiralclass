...
describe("formatPriceForBuyer", () => {
  ...
  it("keeps a market's own currency reading naturally in its own locale", () => {
    ...
    expect(formatPriceForBuyer(150_000, "MXN", "es", "MXN")).toBe("$1,500.00");
  });

  it("still shows the code for a currency that is not the seller's own", () => {
    // ICU sets the code off with a non-breaking space.
    expect(formatPriceForBuyer(150_000, "USD", "es", "MXN")).toBe("USD\u00a01,500.00");
    expect(formatPriceForBuyer(150_000, "MXN", "es")).toBe("MXN\u00a01,500.00");
  });

  it("keeps the disambiguated symbol for an English reader of the seller's own currency", () => {
...
describe("formatMinorUnits", () => {
  ...
  it("renders a non-MXN currency by symbol, with the code exactly once", () => {
    // Regression: the es formatter used to render every currency but MXN by
  ...
  it("respects 3-decimal currencies", () => {
    ...
    // in es's CLDR data, so the formatter emits the code itself and
