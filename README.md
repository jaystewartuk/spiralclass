...
not go wrong twice. More than 125 decision records in
...
cannot `Transfer` to a connected account outside its payout circle, and that is
where the first teachers were. The fix was not to work around the payout circle but to **stop
...
account present SPEI at checkout, and the same buyer was being offered the local
...
the platform's first teacher reads Spanish and sells Spanish lessons to
...
There are more than 800 test files across five layers — unit, real-database integration,
...
| [`docs/decisions/`](docs/decisions/)       | **More than 125 decision records.** Current policy, not history — several reverse an earlier one |
