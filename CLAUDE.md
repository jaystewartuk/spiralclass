...
Writing `gcloud` (`run deploy`, `secrets versions`, service-account keys) is
denied the same way; **reading** it — `describe`, `list`, `logging read` — is
not, because measuring a target is how the decision to use one gets taken.
...
- **Not English-specific and not specific to any one country.** A teacher's subject is a
...
- **The currency-shaped constants are defaults and sentinels, not assumptions** —
  `DEFAULT_PRICING_CURRENCY` is a floor; `teachers.country` is null until
  she picks it. Each is documented where it is
