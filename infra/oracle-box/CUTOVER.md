...
- **The `__LOCAL__` runtime values**, in preview. The deploy script does this and
...
- **Rotate production's LiveKit API key pair, and take preview's away.** The
  pair was also exposed in that transcript, and rotating is cheapest now, while
  `livekit.yaml` is being rewritten anyway. Preview holds **no** LiveKit pair
  from here on ([D-94](../../docs/decisions/D-94.md)'s 2026-09-16 addendum).
  Do it outside lesson hours, because every class in progress drops when
  livekit-server restarts in step 3.
  1. **Generate a new pair** —
     `docker run --rm livekit/livekit-server generate-keys`.
  2. **Set it in Infisical's `production` environment only**: the key at
  ...
  3. **Redeploy the box** (`./scripts/oracle-deploy.sh`). It re-renders
     ...
     production, as in step 8.4.
  4. **Delete preview's `LIVEKIT_API_SECRET` from Infisical**, and preview's
     `LIVEKIT_API_KEY` from `/config`. After step 3 they are dead values, but
     `scripts/docker-entrypoint.sh` lets whatever Infisical holds win over
     `config/env/preview.runtime.env`. Left there, they would put a pair back in
     preview's container on every deploy.
     ⚠️ **Do this only once both the checkout you deploy from and the
     `APP_IMAGE_PREVIEW` image contain the commit that took `LIVEKIT_API_KEY`
     out of `preview.runtime.env`.** Both halves of the deploy's check read
     that file: `render_app_env` reads the checkout's copy, and
     `verify-runtime-env.sh` reads the one baked into the image. If either is
     older, it still expects the key, and the deploy fails with preview
     missing `LIVEKIT_API_KEY`.
     Then **redeploy the box once more**. `preview.app.env` on the box keeps
     the old values until a deploy re-renders it from Infisical.
  5. **Confirm preview holds nothing:**
     `ssh ubuntu@spiralclass-box 'docker exec spiralclass-preview sh -c "env | grep -c ^LIVEKIT_API_"'`
     prints `0`. A class's call page on preview shows "Video calling isn't
     switched on yet". `pnpm livekit:activity` still lists rooms, because it reads
     production's pair.
