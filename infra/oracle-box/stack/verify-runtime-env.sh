...
for container in "$@"; do
  ...
  # ⚠️ THE NAMES GO IN AS AN ENVIRONMENT VARIABLE, NEVER SPLICED INTO THE PROGRAM.
  #
  # `$keys` holds one name per line. Spliced into the text below, it made the
  # loop read `for k in FIRST` then `SECOND; do` on the next line, which every
  # POSIX shell rejects once there are two names. So this check failed on every
  # deploy. A `2>/dev/null` here made the syntax error look like a stopped
  # container, which is why nothing said so. Unquoted, `$VERIFY_KEYS` splits on
  # the newlines inside the container's shell. `set -f` stops a name being read
  # as a glob. stderr is left alone, so the next failure says what it is.
  # apps/web/tests/config/verify-runtime-env.test.ts runs this against a stub docker.
  missing=$(docker exec -e VERIFY_KEYS="$keys" "$container" sh -c '
    set -f
  ...
  ') || {
    echo "UNREACHABLE: $container (running?) — docker exec failed; its error is above" >&2
    rc=1
    continue
  }
