...
#   infisical_export_secrets SESSION_SECRET                    # env=preview (default)
...
# its own options first: the two that source it directly and run
# (`backup-prod-db.sh`, `notify.sh`), and the scripts that reach it through
# another sourced file, which rightly sets no options of its own either — the
# two seed scripts through seed-env.sh, and the two LiveKit activity probes
# through docs/deployment/livekit-credentials.sh.
