...
describe("generateLibraryMaterialForTeacher", () => {
  ...
  // A low-resource language with no captions and no AI generation still
  // resolves to its English name for the prompt.
  it("resolves a low-resource language", async () => {
    teacherRow = { targetLanguage: "qu" };
    await generateLibraryMaterialForTeacher({
      teacherId: "t1",
      topic: "x",
      focusTagIds: [],
    ...
    const seed = (generateMaterial.mock.calls as any)[0][0];
    expect(seed.targetLanguage).toBe("Quechua");
  });
