...
// Shared fixture: Alicia Moreno in San José, Mon–Fri 09:00–13:00 + 16:00–19:00.
...
describe("generateSlots", () => {
  ...
  it("accepts a Date for `date` and resolves weekday in the teacher's timezone", () => {
    // 2026-04-15 05:00 UTC is still 2026-04-14 23:00 in San José (UTC-6,
...
describe("generateSlots — per-rule timezone binding (D-53)", () => {
  ...
  it("anchors slots to the rule's own zone, not the passed teacher zone", () => {
    // Rule frozen in San José; teacher has since relocated and changed their
    // account zone to Madrid. The 09:00 slot must still be 09:00 San José.
    const result = generateSlots(
      base({
  ...
  it("keeps rules and already-booked instants consistent after a teacher relocates", () => {
    // A booking was made at 09:00 San José while the teacher lived there.
    // The teacher then moved to Madrid and changed their account zone. Because
    // the rule stayed frozen in San José, the generator still targets the
...
// transition day — every existing fixture used San José (no DST since
