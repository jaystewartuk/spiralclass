...
vi.mock("@/lib/auth", () => ({
  requireOnboardedTeacher: vi.fn(async () => ({ id: "t1", timezone: "America/Costa_Rica" })),
}));
...
/** A whole local day in San José, the zone the mocked teacher is in. */
function localDay(startYmd: string, endYmd: string) {
  return {
    startsAt: fromZonedTime(`${startYmd}T00:00:00`, "America/Costa_Rica"),
    endsAt: fromZonedTime(`${endYmd}T23:59:59.999`, "America/Costa_Rica"),
  };
}
...
describe("createBlockedDateAction", () => {
  ...
  it("expands the range to local day boundaries and persists the block", async () => {
    ...
    // San José is UTC-6 in July; local midnight → 06:00Z, end-of-day →
