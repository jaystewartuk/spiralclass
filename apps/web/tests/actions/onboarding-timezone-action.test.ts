...
// normalized using the country submitted in the same form, which is required.
...
describe("saveTimezoneAction", () => {
  ...
  it("rejects a form without a country and writes nothing", async () => {
    state.teacher = { id: "t1", country: null };
    const res = await run(fd({ timezone: "America/Costa_Rica", phoneE164: "4155550123" }));
    expect(res).toEqual({ error: "Invalid country" });
  ...
  it("refuses a country change once a live Stripe account exists", async () => {
    ...
    const res = await run(
      fd({ timezone: "America/Costa_Rica", phoneE164: "5512345678", country: "MX" }),
    );
