...
// America/Costa_Rica and a student in Europe/Madrid both saw "12:00 p.m." for
  ...
  return {
    booking: {
      findFirst: async () => ({
        id: BOOKING_ID,
        status: "scheduled",
        // 18:00 UTC — noon in San José (UTC-6), 8:00 p.m. in Madrid
        // (UTC+2, CEST in July).
...
describe("buildVariables renders dates in the actual recipient's zone", () => {
  it("a student-recipient template shows the STUDENT's local time, not the teacher's", async () => {
    ...
    expect(mxCity).toContain("12:00"); // noon in San José
  ...
  it("renders in the recipient's own locale, not a hardcoded default", async () => {
    ...
    // through (previously always defaulted to "es", ignoring the
