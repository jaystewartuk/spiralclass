...
const VARS = {
  ...
  actionCount: 5,
  firstAction: "A useful tip · Riverside Expats",
  minutes: 60,
  dashboardPathSuffix: "dashboard/get-students",
};

function email(languageCode: "es" | "en", overrides: Partial<typeof VARS> = {}) {
  return renderEmail({
...
describe("student_acquisition_plan_teacher — email", () => {
  it("names the first action and the time it should take, in Spanish", () => {
    ...
    expect(out.body).toContain("A useful tip · Riverside Expats");
  ...
  it("does the same in English", () => {
    ...
    expect(out.body).toContain("Start here: A useful tip · Riverside Expats");
  });
