...
function prisma(over: Record<string, unknown> = {}) {
  const base = {
    ...
    teacher: { findMany: vi.fn(async () => [{ id: "t1", locale: "es" }]) },
  };
...
const ACTIVITY = {
  ...
  status: "planned",
  community: { id: "c1", name: "Riverside Expats", url: null, platform: "facebook_group" },
  student: null,
};
...
describe("sendWeeklyPlanNudges", () => {
  ...
  it("names the first action rather than nagging", async () => {
    ...
    expect(metadata.firstAction).toContain("Riverside Expats");
