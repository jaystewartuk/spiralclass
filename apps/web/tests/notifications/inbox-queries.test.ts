...
vi.mock("@/lib/prisma", () => ({
  prisma: {
    teacher: {
      findUnique: vi.fn(async () => ({ name: "Mira", timezone: "America/Costa_Rica" })),
      findMany: vi.fn(async () => [{ id: "t1", name: "Mira", timezone: "America/Costa_Rica" }]),
    },
...
describe("listTeacherInboxPage", () => {
  it("returns nextCursor when an extra row signals more pages", async () => {
    ...
    state.rows = rows(4);
    const page = await listTeacherInboxPage("t1", "es", { take: 3 });
  ...
  it("seeks past a cursor with skip:1", async () => {
    state.rows = rows(1);
    await listTeacherInboxPage("t1", "es", { take: 3, cursor: "n5" });
  ...
  it("omits cursor/skip on the first page", async () => {
    state.rows = rows(1);
    await listTeacherInboxPage("t1", "es", { take: 3 });
  ...
  it("narrows to unread rows in the WHERE clause", async () => {
    state.rows = rows(1);
    await listTeacherInboxPage("t1", "es", { take: 3, unreadOnly: true });
  ...
  it("pages the unread view the same way as the full one", async () => {
    state.rows = rows(4);
    const page = await listTeacherInboxPage("t1", "es", {
      take: 3,
      unreadOnly: true,
      cursor: "n9",
    });
...
describe("listTeacherInbox (array wrapper)", () => {
  it("returns only the items, unchanged for existing callers", async () => {
    state.rows = rows(2);
    const items = await listTeacherInbox("t1", "es");
...
describe("listTeacherInboxPage — payment/package batch preload", () => {
  it("issues one payment.findMany and one package.findMany regardless of row count", async () => {
    ...
    await listTeacherInboxPage("t1", "es", { take: 10 });
  ...
  it("no payment/package-templated rows → neither query fires", async () => {
    ...
    await listTeacherInboxPage("t1", "es");
...
describe("inbox render fan-out", () => {
  it("never renders a full page more than INBOX_RENDER_CONCURRENCY at a time", async () => {
    state.rows = rows(50);
    await listTeacherInboxPage("t1", "es", { take: 50 });
  ...
  it("preserves row order across batch boundaries", async () => {
    ...
    state.rows = rows(12);
    const page = await listTeacherInboxPage("t1", "es", { take: 12 });
