...
describe("saveTemplatesAction — edit path", () => {
  ...
  it("DROPS a Wise price for a teacher with no card rail to discount against", async () => {
    ...
    // used the launch country, because the pre-D-143 card rail was Stripe's cross-border payout
    // circle and that country sat outside it. Direct charges settle on the teacher's
    // own account, so it is now firmly ON the card rail and its teachers'
