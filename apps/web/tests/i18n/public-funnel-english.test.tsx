...
const funnelLocaleForSlug = vi.fn(async () => "es" as const);
...
// (b/[slug]/opengraph-image.tsx) hardcoded its tagline as the es catalog's
...
describe("public booking funnel renders in one language", () => {
  it("renders in her chosen booking-page locale, not the visitor's and not a fixed English", () => {
    ...
    expect(getPublicFunnelT("es")("web.bookingLanding.tagline")).toBe(
      strings["es"]["web.bookingLanding.tagline"],
    );
  ...
  it("declares the funnel's real language with lang=, matching what it renders", async () => {
    ...
    // layout can't reach — so a Spanish browser got lang="es" wrapped around
    ...
    const render = async (locale: "es" | "en") => {
      funnelLocaleForSlug.mockResolvedValueOnce(locale as "es");
    ...
    expect(await render("es")).toContain('lang="es"');
  ...
  it("renders no verbatim Spanish catalog copy anywhere under /b/**", () => {
    // The generalized guard, and it outlives the English pin: any es
    ...
    // coincidence.
    const spanishOnly = Object.entries(strings["es"]).filter(
    ...
    for (const file of funnelSourceFiles(FUNNEL_DIR)) {
      ...
      for (const [key, value] of spanishOnly) {
        if (texts.some((text) => text.includes(value))) {
          offenders.push(`${path.relative(FUNNEL_DIR, file)} hardcodes es copy for "${key}"`);
        }
      }
    }
