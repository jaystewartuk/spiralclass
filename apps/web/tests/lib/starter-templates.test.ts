...
describe("starterTemplatesFor", () => {
  // The regression this pins: the default was es, on the reasoning that
  ...
  it("defaults to the platform locale, not the launch market's", () => {
    ...
    expect(starterTemplatesFor()).not.toEqual(starterTemplatesFor("es"));
  });
  ...
  it("still localizes names to Spanish when given locale 'es'", () => {
    expect(starterTemplatesFor("es").map((t) => t.name)).toEqual([
      "4 clases / 1 mes",
      "8 clases / 2 meses",
      "12 clases / 3 meses",
      "20 clases / 5 meses",
  ...
  it("localizes names to English when given locale 'en', without changing the amounts", () => {
    ...
    expect(templates.map(({ name: _name, ...rest }) => rest)).toEqual(
      starterTemplatesFor("es").map(({ name: _name, ...rest }) => rest),
    );
  });
