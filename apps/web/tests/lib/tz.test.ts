...
describe("zonedWallClockToUtc", () => {
  it("returns a UTC Date matching the wall-clock in the given zone (no DST)", () => {
    ...
    const d = zonedWallClockToUtc("2026-04-15", "09:00", CR);
  ...
  it("agrees with date-fns-tz on the inverse direction", () => {
    const ours = zonedWallClockToUtc("2026-04-15", "17:30", CR);
    const theirs = fromZonedTime("2026-04-15T17:30:00", CR);
...
describe("toYMD", () => {
  it("returns YYYY-MM-DD in the given zone", () => {
    ...
    expect(toYMD(utc, CR)).toBe("2026-04-15");
  });
  ...
  it("zero-pads month and day to two digits", () => {
    ...
    expect(toYMD(d, CR)).toBe("2026-01-05");
  });
...
describe("weekdayInZone", () => {
  it("returns 0 for Sunday … 6 for Saturday in the given zone", () => {
    ...
    const monNoon = zonedWallClockToUtc("2026-04-13", "12:00", CR);
    expect(weekdayInZone(monNoon, CR)).toBe(1);

    const sunNoon = zonedWallClockToUtc("2026-04-12", "12:00", CR);
    expect(weekdayInZone(sunNoon, CR)).toBe(0);

    const satNoon = zonedWallClockToUtc("2026-04-18", "12:00", CR);
    expect(weekdayInZone(satNoon, CR)).toBe(6);
  });

  it("disagrees across zones when the instant crosses midnight", () => {
    // 2026-04-13 23:00 CDMX (Monday) = 2026-04-14 05:00 UTC (Tuesday)
    const lateMonInCr = zonedWallClockToUtc("2026-04-13", "23:00", CR);
    expect(weekdayInZone(lateMonInCr, CR)).toBe(1); // Mon
    expect(weekdayInZone(lateMonInCr, "UTC")).toBe(2); // Tue
  });
});

describe("normalizeInputDate", () => {
  it("passes strings through unchanged", () => {
    expect(normalizeInputDate("2026-04-15", CR)).toBe("2026-04-15");
  });

  it("converts Date objects to YMD in the requested zone", () => {
    const utc = new Date("2026-04-16T03:00:00Z"); // 21:00 on 04-15 CDMX
    expect(normalizeInputDate(utc, CR)).toBe("2026-04-15");
...
describe("formatDateTimeInZone", () => {
  it("renders a human-readable date+time string in the teacher's zone", () => {
    ...
    const out = formatDateTimeInZone(d, CR);
  ...
  it("honors an explicit locale override", () => {
    ...
    const out = formatDateTimeInZone(d, CR, "en-US");
  ...
  it("flips the date when crossing zones changes the day", () => {
    ...
    expect(formatDateTimeInZone(d, CR)).toMatch(/april 15/i);
