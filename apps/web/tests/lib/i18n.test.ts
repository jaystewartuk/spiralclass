...
// value (`"en"`, `"es"`, or undefined for "not set"); `accept` is the
...
describe("getPreferredLocale", () => {
  ...
  it("returns the `locale` cookie when set to a known value", async () => {
    setup({ cookie: "en", accept: "es;q=0.9" });
    const { getPreferredLocale } = await import("@/lib/i18n");
    expect(await getPreferredLocale()).toBe("en");
  });

  it("returns es when the cookie is set to es (header is ignored)", async () => {
    setup({ cookie: "es", accept: "en-US" });
    const { getPreferredLocale } = await import("@/lib/i18n");
    expect(await getPreferredLocale()).toBe("es");
  });

  it("ignores cookies set to unknown values and falls through to header", async () => {
    setup({ cookie: "xx-YY", accept: "es;q=0.9" });
    const { getPreferredLocale } = await import("@/lib/i18n");
    expect(await getPreferredLocale()).toBe("es");
  });

  it("reduces a regional cookie to its app locale", async () => {
    setup({ cookie: "fr-FR", accept: "es;q=0.9" });
    const { getPreferredLocale } = await import("@/lib/i18n");
    expect(await getPreferredLocale()).toBe("fr");
  });

  it("returns es when no cookie + Accept-Language starts with es", async () => {
    setup({ accept: "es,es;q=0.9" });
    const { getPreferredLocale } = await import("@/lib/i18n");
    expect(await getPreferredLocale()).toBe("es");
  });
