...
describe("timezoneSchema", () => {
  ...
  it("accepts valid IANA timezones", () => {
    expect(
      timezoneSchema().safeParse({ timezone: "America/Costa_Rica", phoneE164: wa, country: "MX" })
        .success,
    ).toBe(true);
    expect(
      timezoneSchema().safeParse({ timezone: "America/Tijuana", phoneE164: wa, country: "MX" })
        .success,
    ).toBe(true);
    expect(
      timezoneSchema().safeParse({ timezone: "Europe/London", phoneE164: wa, country: "GB" })
        .success,
    ).toBe(true);
  });
  ...
  it("requires a valid E.164 phone number", () => {
    ...
    expect(
      timezoneSchema().safeParse({ timezone: "America/Costa_Rica", phoneE164: "" }).success,
    ).toBe(false);

    // Missing entirely is rejected too.
    expect(timezoneSchema().safeParse({ timezone: "America/Costa_Rica" }).success).toBe(false);
  });
