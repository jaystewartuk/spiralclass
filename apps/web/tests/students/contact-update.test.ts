...
describe("student self-edit", () => {
  ...
  it("resolves a bare national number to the student's picked country, not the +52 default", async () => {
    const result = await applyStudentContactUpdate({
      studentId: STUDENT_ID,
      actor: { type: "student" },
  ...
  it("falls back to the +52 default when no phoneCountry hint is given", async () => {
    const result = await applyStudentContactUpdate({
      studentId: STUDENT_ID,
      actor: { type: "student" },
