...
// 2026-09-01 20:00Z is 14:00 in San José — comfortably mid-afternoon, so
...
function row(over: Partial<UpcomingRow> & { id: string; scheduledStart: Date }): UpcomingRow {
  return {
    student: { name: "María González", timezone: "America/Costa_Rica" },
  ...
  it("renders the soonest class in the teacher's own zone and marks it next up", async () => {
    ...
    upcoming = [
      ...
      row({
        ...
        student: { name: "Diego Ruiz", timezone: "America/Costa_Rica" },
      }),
    ];
    ...
    // 22:00Z is 16:00 in San José — the TEACHER's clock, which is the whole
    ...
    expect(out).toContain("Times shown in Costa Rica.");
    expect(out).not.toContain("America/Costa_Rica");
  });
