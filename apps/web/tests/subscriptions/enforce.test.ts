...
describe("upgradeNudge", () => {
  ...
  it("speaks French rather than falling through to Spanish", () => {
    ...
    expect(fr).not.toBe(upgradeNudge("students", "es"));
    expect(upgradeNudge("materials", "fr")).not.toBe(upgradeNudge("materials", "es"));
    expect(upgradeNudge("homework_review", "fr")).not.toBe(upgradeNudge("homework_review", "es"));
  });
