...
describe("planLabel", () => {
  it("renders Spanish labels", () => {
    expect(planLabel("free", "es")).toBe("Gratis");
    expect(planLabel("monthly", "es")).toBe("Pro Mensual");
    expect(planLabel("annual", "es")).toBe("Pro Anual");
    expect(planLabel("founding", "es")).toBe("Fundador");
  });
...
describe("statusLabel", () => {
  it("renders Spanish statuses", () => {
    expect(statusLabel("trialing", "es")).toBe("Prueba");
    expect(statusLabel("active", "es")).toBe("Activa");
    expect(statusLabel("past_due", "es")).toBe("Pago vencido");
    expect(statusLabel("canceled", "es")).toBe("Cancelada");
    expect(statusLabel("free", "es")).toBe("Gratis");
  });
...
describe("planPriceLabel", () => {
  ...
  it("suffixes a monthly cadence for monthly and founding plans", () => {
    expect(planPriceLabel("monthly", "es")).toBe("£7.99 GBP/mes");
  ...
  it("suffixes a yearly cadence for the annual plan", () => {
    expect(planPriceLabel("annual", "es")).toBe("£79.90 GBP/año");
  ...
  it("shows free without a cadence suffix", () => {
    expect(planPriceLabel("free", "es")).toBe("Gratis");
