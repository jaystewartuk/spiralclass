...
describeIntegration("invitation manage (real DB)", () => {
  ...
  it("defaults a new roster student's locale to the column default, not es", async () => {
    const prisma = getTestPrisma();
    const res = await provisionInvitation({
      teacherId: TEACHER_ID,
      email: "loc@e2e.test",
  ...
  it("honours an explicit locale on a new roster student", async () => {
    ...
    if (res.status !== "created") throw new Error("setup");
    expect((await prisma.student.findUnique({ where: { id: res.studentId } }))?.locale).toBe("es");
    expect(res.studentLocale).toBe("es");
  });

  it("reports an EXISTING roster student's own locale, so the invite email follows it", async () => {
    // The regression: a CSV-imported student stamped es (or an English
    ...
    const res = await provisionInvitation({
      ...
      email: "english@e2e.test",
      name: null,
      // Even with an explicit es for NEW rows, an existing row wins.
      locale: "es",
    });
    if (res.status !== "created") throw new Error("setup");
