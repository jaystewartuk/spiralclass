...
// Group-level error boundary for the (app) teacher surface. Keeps the throw
...
export default function AppError({
  ...
  return (
    <main className="container flex min-h-dvh flex-col items-center justify-center gap-6 py-12 text-center">
      <Link href="/dashboard" aria-label={t("common.brandName")}>
        <Logo size="md" />
      </Link>
      <div className="max-w-md space-y-3">
        <Heading level={1}>{t("common.error")}</Heading>
        <p className="text-muted-foreground">{t("web.errorBoundary.app.body")}</p>
        {error.digest && (
      ...
      <div className="flex flex-col gap-3 lg:flex-row">
        ...
        <Button asChild size="lg" variant="outline">
          <Link href="/dashboard">{t("web.errorBoundary.app.home")}</Link>
        </Button>
      </div>
    </main>
  );
