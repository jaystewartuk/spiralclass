...
async function createCheckoutIntentCore(
  ...
  // better guess than the module's last-resort default for a teacher outside
  // the +52 default. If a phone field returns here, wire a country picker
  // alongside it (leads.ts does) so the student declares her own instead.
  const phoneE164 = input.studentPhone
    ? normalizeE164(input.studentPhone, teacher.country ?? undefined)
    : null;
