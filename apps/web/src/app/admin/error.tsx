...
// Group-level error boundary for the /admin surface. Keeps the throw from
// bubbling to the root boundary and losing the admin shell.
...
export default function AdminError({
  ...
  return (
    <main className="container flex min-h-dvh flex-col items-center justify-center gap-6 py-12 text-center">
      <Link href="/admin" aria-label={t("common.brandName")}>
        <Logo size="md" />
      </Link>
      ...
      <div className="flex flex-col gap-3 lg:flex-row">
        ...
        <Button asChild size="lg" variant="outline">
          <Link href="/admin">{t("web.admin.errorBoundary.home")}</Link>
        </Button>
      </div>
    </main>
  );
