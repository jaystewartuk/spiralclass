...
// Photo attachments upload direct-to-R2 (see lib/storage/chat-media.ts):
// mirrors the teacher-side route. See lib/storage/chat-media.ts.
export async function GET(
  ...
  const presigned = presignChatMedia("image", teacherId, ts.studentId, contentType, Date.now());
...
export async function POST(
  ...
  const check = await validateChatMediaUpload("image", storagePath, teacherId, ts.studentId);
  if (!check.ok)
    return NextResponse.json(
