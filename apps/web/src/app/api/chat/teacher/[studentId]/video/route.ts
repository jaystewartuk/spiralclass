...
export async function GET(
  ...
  const presigned = presignChatMedia("video", teacher.id, studentId, contentType, Date.now());
...
export async function POST(
  ...
  const check = await validateChatMediaUpload("video", storagePath, teacher.id, studentId);
  if (!check.ok)
    return NextResponse.json(
