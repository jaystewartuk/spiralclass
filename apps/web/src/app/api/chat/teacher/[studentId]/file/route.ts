...
// Document attachments upload direct-to-R2 (see lib/storage/chat-media.ts):
// GET presigns a PUT ticket, the browser uploads straight to R2, then POST
// finalizes. Mirrors image/route.ts, plus persists the original file name.
export async function GET(
  ...
  const presigned = presignChatMedia("file", teacher.id, studentId, contentType, Date.now());
...
export async function POST(
  ...
  const check = await validateChatMediaUpload("file", storagePath, teacher.id, studentId);
  if (!check.ok)
    return NextResponse.json(
