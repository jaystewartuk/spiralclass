...
// Photo attachments upload direct-to-R2 (see lib/storage/chat-media.ts): GET
// presigns a PUT ticket, the browser uploads straight to R2, then POST
// finalizes. Mirrors voice/route.ts.
export async function GET(
  ...
  const presigned = presignChatMedia("image", teacher.id, studentId, contentType, Date.now());
...
export async function POST(
  ...
  const check = await validateChatMediaUpload("image", storagePath, teacher.id, studentId);
  if (!check.ok)
    return NextResponse.json(
