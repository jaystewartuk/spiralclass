...
// for one selling out of it: the first live teacher sells Spanish to English
// speakers, so every visitor was handed her country's phone picker and
// timezone placeholder — a small, early signal that the page is not for them.
//
...
function leadPhoneCountryFor(
  funnelLocale: string,
  teacherCountry: string | null,
): string | undefined {
  try {
    return new Intl.Locale(funnelLocale).maximize().region ?? teacherCountry ?? undefined;
  } catch {
    return teacherCountry ?? undefined;
  }
}
...
export async function generateMetadata({
  ...
  const priceBit =
    from != null
      ? ` ${t("web.bookingLanding.fromPrice", { price: formatPriceForBuyer(from, cheapestTemplate.currency, funnelLocale, cheapestTemplate.currency) })}`
      : "";
...
export default async function BookingLandingPage({
  ...
  // proxy for the audience here: the first live teacher sells Spanish to
  // English speakers, many of them living in her country, so a local IP says
