...
// Inline photo attachments in the teacher↔student chat — same R2 bucket as
// chat audio/video ("chat-audio" / CHAT_AUDIO_R2_*), under an `image/` key
// prefix. Mirrors chat-video.ts exactly; see that file for the rationale on
...
function r2Config(): R2Config | null {
  ...
  if (!bucket || !endpoint || !accessKey || !secret) {
    ...
    console.error("[chat-image] r2 not configured — missing:", missing.join(", "));
    return null;
  }
  let host: string;
  try {
    ...
    console.error("[chat-image] CHAT_AUDIO_R2_ENDPOINT is not a valid URL:", endpoint);
    return null;
  }
...
/** Mint a presigned PUT URL + server-chosen storage path for a new image. */
export function presignChatImageUpload(
  teacherId: string,
  studentId: string,
  contentType: string,
  timestamp: number,
  ...
  const storagePath = `image/${teacherId}/${studentId}/${timestamp}.${ext}`;
...
/** Guard a client-supplied image path on finalize: exactly under this thread's
 *  `image/<teacher>/<student>/` prefix, no traversal, allowed extension. */
export function isChatImagePath(
  storagePath: string,
  teacherId: string,
  studentId: string,
): boolean {
  const prefix = `image/${teacherId}/${studentId}/`;
  ...
  return Object.values(ALLOWED_IMAGE_TYPES).includes(ext);
}

/** HEAD the uploaded object: confirm it exists and read its size. Returns
 *  byte size, or null if missing. */
export async function headChatImageObject(storagePath: string): Promise<number | null> {
  const cfg = r2Config();
  if (!cfg) return null;
...
export async function mintChatImageSignedUrl(storagePath: string): Promise<string | null> {
  const cfg = r2Config();
  if (!cfg) return null;
...
/** Delete an image object when its message is deleted-for-everyone. Best
 *  effort and idempotent — see deleteChatAudioObject for the rationale. */
export async function deleteChatImageObject(storagePath: string): Promise<boolean> {
  const cfg = r2Config();
  if (!cfg) return false;
  const url = presignS3Url({
    accessKeyId: cfg.accessKey,
