...
export function buildContentSystemPrompt(input: ContentPromptInput): string {
  ...
  return [
    `You write social posts for ONE independent teacher. You are not a marketing agency and you must never sound like one.`,
    ``,
...
export function buildContentUserPrompt(input: ContentPromptInput): string {
  ...
  const lines: string[] = [
    ...
    ctx.country ? `Based in: ${ctx.country}` : `Based in: not set.`,
