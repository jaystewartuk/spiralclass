...
// that bias is reversed is in-app browsers — see HUMAN_IN_APP_BROWSERS — and a
// prefetch from one is caught by its purpose header instead, see
// SPECULATIVE_PURPOSE_HEADERS.
...
/**
 * Headers a client sends when it fetches a page speculatively — ahead of a tap
 * that may never come — and the purposes in them that mean so.
 *
 * Facebook's Android app prefetches links as they scroll into view, from its
 * in-app browser, with the in-app agent and `X-Purpose: preview`. The agent is
 * exactly a person's, so HUMAN_IN_APP_BROWSERS rightly lets it through, and
 * only this header tells the prefetch from the tap. Chrome sends `Sec-Purpose:
 * prefetch` (with `;prerender` or `;anonymous-client-ip` parameters), older
 * Chrome `Purpose: prefetch`, and Firefox `X-Moz: prefetch`.
 *
 * The trade: a tap the app then serves from its prefetch cache never reaches
 * the server, so that visit counts zero times rather than once. That is the
 * bias this file already states — an uncounted visit is cheaper than an
 * inflated one. Like the agent, the header is read and discarded (D-125).
 */
const SPECULATIVE_PURPOSE_HEADERS = ["x-purpose", "purpose", "sec-purpose", "x-moz"];
const SPECULATIVE_PURPOSES = ["prefetch", "preview"];

/**
 * Whether a request announces itself as a prefetch or preview rather than a
 * page someone opened. Separate from the agent check on purpose: it is what
 * excludes a prefetch from an in-app browser without reclassifying the in-app
 * browser itself.
 */
export function isSpeculativeRequest(headers: Pick<Headers, "get">): boolean {
  return SPECULATIVE_PURPOSE_HEADERS.some((name) => {
    const value = headers.get(name)?.toLowerCase() ?? "";
    return SPECULATIVE_PURPOSES.some((purpose) => value.includes(purpose));
  });
}

/**
 * Whether the request currently being served should stay out of the visit
 * count: a bot by its agent, or a speculative fetch by its purpose header.
 *
...
export async function currentRequestIsBot(): Promise<boolean> {
  try {
    ...
    return isSpeculativeRequest(store) || isBotUserAgent(store.get("user-agent"));
  } catch {
    return false;
  }
}
