...
describe("groupPaymentsByMonth", () => {
  ...
  it("files a payment by the TEACHER's calendar month, not UTC's", () => {
    // 7pm on 31 August in San José is 1 September in UTC. Filing it under
    // September puts it on the wrong side of the line she does her books by.
    const months = groupPaymentsByMonth([entry("2026-09-01T01:00:00Z")], "America/Costa_Rica");
...
describe("monthKey", () => {
  it("is the teacher's calendar month, not the UTC one", () => {
    expect(monthKey(new Date("2026-09-01T01:00:00Z"), "America/Costa_Rica")).toBe("2026-08");
