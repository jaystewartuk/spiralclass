...
const DOCS: ContentDoc[] = [
  {
    ...
    audience: "teacher",
    title: { en: "Manage settings and your plan", es: "Administra tu configuración" },
    summary: { en: "Update your profile and plan.", es: "Actualiza tu perfil." },
    body: {
      en: [
        "## Purpose",
        "",
...
describe("prepareHelpGuides", () => {
  ...
  it("localizes title, summary and body together", () => {
    const es = prepareHelpGuides(DOCS, "es", anchorLinkResolver(DOCS));
    expect(es[0].title).toBe("Administra tu configuración");
    expect(es[0].summary).toBe("Actualiza tu perfil.");
    // The es body's only section IS the summary, so the echo rule removes
