 ...
 * nor zoned — a package expiring at 23:00 in a UTC-6 zone printed the following
...
export function formatZonedDateCompact(
  ...
  return new Intl.DateTimeFormat(intlLocale(locale), { timeZone: tz, dateStyle: "medium" }).format(
    d,
  );
}
...
/**
 * The human half of an IANA zone id: `America/Costa_Rica` → "Costa Rica".
 *
 ...
 * string `America/Costa_Rica`. The zone still has to be NAMED (this product
...
export type ZoneNow = {
  ...
  date: string;
  /** The zone's locality, as a person would say it: "Buenos Aires". */
  city: string;
