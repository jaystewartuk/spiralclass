...
export function formatThreadTimestamp(
  ...
  if (days === 0)
    return at.toLocaleTimeString(intlLocale(locale), { ...timeOptionsFor(locale), timeZone: tz });
  if (days === 1) return t("chat.day.yesterday");
  if (days < 7)
    return at.toLocaleDateString(intlLocale(locale), { weekday: "short", timeZone: tz });
  return at.toLocaleDateString(intlLocale(locale), {
    day: "numeric",
    month: "short",
