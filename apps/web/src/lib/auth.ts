...
export const requireTeacher = cache(async () => {
  ...
  // starter-templates.ts for why that fallback is no longer es.
...
export async function requireOnboardedTeacher() {
  ...
  // The timezone step is where her country is captured, so an onboarded
  // teacher always has one; the type says so for every caller.
  if (!teacher.onboardingCompleteAt || !teacher.country) redirect("/onboarding/timezone");
  return { ...teacher, country: teacher.country };
}
