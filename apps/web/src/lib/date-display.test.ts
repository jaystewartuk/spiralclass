...
// 16:00 UTC — 10:00 a.m. in San José (UTC-6), 5:00 p.m. in London (BST, UTC+1).
...
describe("bookingWhen", () => {
  ...
  it("is symmetric: swapping viewer/other swaps which time is primary", () => {
    ...
    const asTeacher = bookingWhen(
      SCHEDULED_START,
      "Europe/London",
      { tz: "America/Costa_Rica", label: "Student" },
      "en",
      t,
    );
  ...
  it("still renders a secondary line when both parties share a timezone", () => {
    const result = bookingWhen(
      SCHEDULED_START,
      "America/Costa_Rica",
      { tz: "America/Costa_Rica", label: "Alicia Moreno" },
      "en",
      t,
    );
    expect(result.whenSecondary).toContain("Alicia Moreno");
  ...
  it("renders in Spanish when given the es translator", () => {
    const result = bookingWhen(
      SCHEDULED_START,
      "America/Costa_Rica",
      { tz: "Europe/London", label: "Alicia Moreno" },
  ...
  it("handles a midnight-boundary date difference between viewer and other", () => {
    // 23:30 UTC -> San José still same day 17:30, Tokyo already next day 08:30.
...
describe("timezoneCityLabel", () => {
  it("renders the locality of a two-part zone id", () => {
    expect(timezoneCityLabel("America/Costa_Rica")).toBe("Costa Rica");
...
describe("formatZonedShortDate", () => {
  ...
  // previous evening in San José, and the badge has to say the day the
  ...
  it("reads the day in the given zone, not in UTC", () => {
    ...
    expect(formatZonedShortDate(justAfterMidnightUtc, "America/Costa_Rica", "en", now)).toContain(
      "12",
    );
  ...
  it("compares years in the target zone rather than the runtime's", () => {
    const newYearsEveInCostaRica = new Date("2026-01-01T04:00:00Z");
    const laterThatYear = new Date("2026-06-01T12:00:00Z");
    expect(
      formatZonedShortDate(newYearsEveInCostaRica, "America/Costa_Rica", "en", laterThatYear),
    ).toContain("2025");
  });

  it("formats in the requested locale", () => {
    const august = new Date("2026-08-12T16:00:00Z");
    expect(formatZonedShortDate(august, "UTC", "es", now)).toContain("ago");
...
describe("zoneNow", () => {
  // 02:00 UTC on the 16th is still the evening of the 15th in San José. The
  ...
  it("renders the wall clock, the locality and the calendar date of one zone", () => {
    const clock = zoneNow(instant, "America/Costa_Rica", "en");
    ...
    expect(clock.city).toBe("Costa Rica");
  ...
  it("puts two zones on different calendar days when they are", () => {
    expect(zoneNow(instant, "America/Costa_Rica", "en").ymd).toBe("2026-07-15");
  ...
  it("formats the time in the VIEWER's locale, not the zone's country", () => {
    ...
    expect(spaces(zoneNow(instant, "America/Costa_Rica", "fr").time)).toBe("20:00");
  });
  ...
  it("follows a zone across its own DST boundary", () => {
    ...
    // Costa Rica has no DST — CST all year, so its clock does not move.
    expect(spaces(zoneNow(summer, "America/Costa_Rica", "en").time)).toBe("10:00 AM");
    expect(spaces(zoneNow(winter, "America/Costa_Rica", "en").time)).toBe("10:00 AM");
  });
});
...
describe("sameWallClock", () => {
  ...
  it("is false when the hour differs", () => {
    expect(sameWallClock(at("Europe/London"), at("America/Costa_Rica"))).toBe(false);
  });
  ...
  it("is false when only the calendar date differs", () => {
    ...
    const a = zoneNow(midnight, "America/Costa_Rica", "en");
...
// package expiring late in the evening in San José showed the NEXT day,
...
describe("formatZonedDateCompact", () => {
  it("resolves the date in the target zone, not UTC", () => {
    // 2026-10-04T04:00Z is still 2026-10-03 in San José: the bug.
    const late = new Date("2026-10-04T04:00:00Z");
    expect(formatZonedDateCompact(late, "America/Costa_Rica", "en")).toContain("3");
  ...
  it("gives a locale its own date order rather than a translated English one", () => {
    ...
    const es = formatZonedDateCompact(d, "UTC", "es");
