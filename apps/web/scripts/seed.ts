...
const SEED_TZ = "America/Costa_Rica";
// Every onboarded teacher has a country (the onboarding timezone step captures
// it, and requireTeacher sends a teacher without one back there), so every
// seeded teacher that skips onboarding needs one set. Matches the seed's MXN
// pricing and the bank schemes its rail fixtures exercise.
const SEED_COUNTRY = "MX";
...
type HeroSpec = {
  ...
  // behaviour apply. Defaults to SEED_COUNTRY.
  country?: string;
...
export function heroSpecs(): HeroSpec[] {
  return [
    ...
    },
    {
      ...
      // She sells to English speakers, like the first live teacher — the
...
async function seedHero(
  ...
  const teacher = await prisma.teacher.create({
    data: {
      ...
      bio: `${hero.headline}.`,
      onboardingCompleteAt: now,
      country: hero.country ?? SEED_COUNTRY,
