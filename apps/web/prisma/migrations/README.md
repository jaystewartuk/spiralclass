...
The baseline is **regenerated, never edited**. `schema.prisma` beside it is
the model it was cut from — the live `prisma/schema.prisma` has moved on by
every migration since — and

```bash
pnpm --filter spiralclass-web exec prisma migrate diff \
  --from-empty --to-schema prisma/migrations/20260906120000_schema_baseline/schema.prisma --script
```
...
4. **Make it work with the code already serving** ([D-178](../../../../docs/decisions/D-178.md)).
   A production deploy migrates before its new code is live, and the Cloud Run job
